...
The Clerk publishable key, CLI OAuth client ID, and relay URL are public identifiers, not secrets.
...
should set `T3CODE_CLERK_PUBLISHABLE_KEY`, `T3CODE_CLERK_CLI_OAUTH_CLIENT_ID`, and `T3_RELAY_URL`
before building. EAS preview and production builds only need the Clerk publishable key and relay URL
in their EAS environment.

When the relevant public values are absent, cloud UI and the `t3 cloud` CLI command group are
omitted. The bundled server still accepts runtime overrides for self-hosted or operator-managed
deployments.
